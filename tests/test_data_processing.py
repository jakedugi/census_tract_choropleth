...
import pandas as pd
import os
from data_processing import process_acs_csv, process_csv, print_population_stats, clean_data
...
def test_process_acs_csv(tmp_path, sample_acs_data, column_mapping):
    ...
    assert processed_df['Total_Population'].dtype == 'float64'  # Check numeric conversion
    assert processed_df['Total_Population'].iloc[0] == 1000.0  # Check value preservation
...
def test_print_population_stats(capsys):
    ...
    df = pd.DataFrame({
        'Total_Population': [100, 200, 300, 400, 500]
    })
